      ...
      {(filters.search || filters.category || filters.new || filters.range ? filteredProducts : products).map((item) => {
        ...
        return (
          <Link key={id} to={`/products/${id}`} className="card card-compact sm:max-w-96 shadow-xl hover:scale-105 duration-200">
            <figure>{images?.length > 0 ? <img src={`https://seller-project-server-1.onrender.com/uploads/${images[0]}`} alt={title} className="w-full h-64 md:h-48 lg:h-48 xl:h-60 object-cover" /> : <BiImageAdd className="w-full h-64 md:h-48 xl:h-60 " />}</figure>
